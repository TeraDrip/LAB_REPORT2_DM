...
class SalonETL:
    ...
    def check_supabase_connection(self, test_table=None):
        ...
        try:
            if test_table:
                self.supabase.table(test_table).select("*").limit(1).execute()
                print(f"✓ SUPABASE CONNECTED: Successfully connected to table: {test_table}")
            else:
                print("✓ SUPABASE CLIENT READY: Client initialized (No table test performed).")
            return True
    ...
    def extract_from_cloud(self, table_name):
        ...
        try:
            ...
            print(f"✓ EXTRACT SUCCESS: Retrieved {len(response.data)} records from {table_name}.")
...
if __name__ == "__main__":
    ...
    # Replace 'customer_transactions' with your actual table name to see a live test
