...
class CSVToSupabaseETL:
    ...
    def resolve_db_url(self) -> str | None:
        def sanitize_postgres_dsn(raw_dsn: str) -> str:
            ...
            # psycopg doesn't accept some libpq-unrecognized flags used by other clients.
    ...
    def process_file(self, csv_path: Path, table_name_override: str | None = None) -> None:
        table_name = sanitize_identifier(table_name_override or csv_path.stem, "table")
        ...
        if self.use_postgres:
            try:
                ...
                print(f"[WARN] Direct Postgres load failed for {table_name}: {exc}")
                print(
                    "[HINT] Falling back to Supabase REST API. "
                    "Set SUPABASE_DB_URL/SUPABASE_DB_PASSWORD correctly if you want direct DB mode."
                )

                # Try REST fallback when API credentials are available.
                if self.supabase_url and self.supabase_key:
                    max_attempts = 4
                    for attempt in range(1, max_attempts + 1):
                        try:
                            self.load_to_supabase_rest(table_name, transformed_df)
                            print(f"[OK] {csv_path.name} -> public.{table_name} ({len(transformed_df)} rows via REST)")
                            return
                        except Exception as rest_exc:
                            is_schema_cache_issue = "PGRST205" in str(rest_exc)
    ...
    def run(self, table_name_override: str | None = None) -> None:
        ...
        for csv_file in csv_files:
            file_table_name = table_name_override if len(csv_files) == 1 else None
            self.process_file(csv_file, table_name_override=file_table_name)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-path",
        default=str(Path(__file__).resolve().parent / "csv"),
        help="Path to a CSV file or a directory to recursively scan for CSV files",
    )
    parser.add_argument(
        "--table-name",
        default=None,
        help="Optional explicit table name (sanitized by ETL) for single-file runs",
    )
...
if __name__ == "__main__":
    ...
    etl.run(table_name_override=args.table_name)
